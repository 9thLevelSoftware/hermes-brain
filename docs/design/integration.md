...
- `export [--out dir] [--full]` / `import <file> [--trust-owner]` — plain-file exports into `~/.hermes/brain/exports/<date>/`. **Two modes, and the distinction is load-bearing:** the default writes `memories.jsonl` (current truth only) plus a human-readable markdown tree (`profile.md`, `warnings.md`, `topics/*.md`); `--full` writes a `manifest.json` plus every table — memories **including superseded and tombstoned versions**, episodes, facts, entities, entity_mentions, edges — and `import` restores all of it with uids preserved (idempotent; existing uids are skipped). Shipped without the sketched `--format`: the two default formats are complementary, so `export` always writes both.

  > **CORRECTED 2026-07-26 (alignment-audit.md §G3).** This line previously described the default export as "lossless, re-importable". It was neither: it dropped every episode, fact triple, entity, edge, and all version history. `--full` is the mode that makes the claim true.
