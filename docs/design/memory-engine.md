...
FLUSH (background worker, serialized — Hermes already guarantees single worker):
  Phase 1  batched LLM extraction (1 call)  → candidates{type, lane=evidence, content, entities,
           relations, happened_at?, retention, ttl_days?, supersedes_uid?, scope hints}
...
**2.2 Batched extraction (Memobase).** One LLM call per flush over the buffered spans (via Hermes MCP sampling or auxiliary provider; stronger model welcome, it's background). Prompt v3 emits **only** memory-worthy items and classifies `retention` as `episode_only|temporary|durable`; temporary items require integer `ttl_days` from 1–365. Commit/process narration is episode-only, while branch/deployment/PR/CI/current-work state defaults to seven days. Explicit dated state expires one day after its stated end date. Preferences, warnings, profiles, and durable decisions receive no inferred TTL unless explicitly temporary. The extractor may nominate one bounded current candidate via `supersedes_uid`; nomination alone never authorizes correction. Floor tier (no LLM available): deterministic capture only — `on_memory_write` mirrors and explicit `brain_remember` calls; no synthetic candidates.
...
**2.7 Incognito / TTL (provable bypass).** The incognito check is at the single capture entry point: if the session is incognito, capture returns before archive or buffer writes. Current truth everywhere is the same predicate: `valid_to IS NULL`, `status='active'`, `live=1`, and `ttl_at IS NULL OR ttl_at > now`. Query-time filtering is authoritative even when dream has not run. A lifecycle pass before lane-1 materialization, dream, and retrieval maintenance marks due rows `expired`, closes `valid_to`, removes current vectors/fact/edge participation, and bumps cache generations. It preserves content, provenance, and version lineage for history and restore. A high-precision legacy pass assigns bounded TTLs to operational patterns; uncertain model-only classifications remain shadow proposals.

---

## 3. Read path

Entry points: `prefetch(query)` (effective lane-2 default 400 tokens under the combined 800-token Brain cap, injected into the per-turn `<memory-context>` fence), and the `brain_recall` tool. Current-truth paths all apply the centralized lifecycle predicate; direct ID history lookup deliberately does not.
